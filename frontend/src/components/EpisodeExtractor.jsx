...
import { 
  ...
  ChevronDown, Copy, Check, Globe, Upload, Trash2, Plus, Save
} from 'lucide-react';
...
export default function EpisodeExtractor({ fetchScripts, scripts, activeScript, onSelectScript, onDeleteScript }) {
  ...
  const handleCopyWholeScript = () => {
    if (!generatedScript) return;
    const textBlocks = [];
    textBlocks.push(`=== EXTRACTED SUMMARY: ${generatedScript.animeTitle} ===`);
  ...
  const handleUploadSubmit = async (e) => {
    ...
    formData.append('narrativeMode', narrativeMode);
        ...
        <div className="glass-panel rounded-2xl p-6 border-cyan-500/20 shadow-neon-cyan relative overflow-hidden">
          ...
          <form onSubmit={sourceTab === 'upload' ? handleUploadSubmit : handleOnlineSubmit} className="space-y-4">
            ...
            <div className="text-left">
              <label className="block text-[10px] font-black text-cyber-cyan uppercase tracking-wider mb-1.5">
                Extraction Narrative Mode
              </label>
              <select
                value={narrativeMode}
                onChange={(e) => setNarrativeMode(e.target.value)}
                disabled={isLoading}
                className="w-full bg-slate-950/80 border border-slate-800 focus:border-cyber-cyan rounded-xl px-4 py-3 text-xs text-white focus:outline-none transition-all"
              >
