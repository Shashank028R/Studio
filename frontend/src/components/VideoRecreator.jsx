...
import { 
  ...
  Settings, CheckCircle, Upload, Trash2, Globe,
  ChevronDown, Copy, Check, Video, RefreshCw
} from 'lucide-react';
...
export default function VideoRecreator({ fetchScripts, scripts, activeScript, onSelectScript, onDeleteScript }) {
  ...
  useEffect(() => {
    if (activeScript && activeScript.videoType === 'translated') {
  ...
  const handleRecreateSubmit = async (e) => {
    ...
    setIsLoading(true);
    setError('');
    setSuccessMsg('Extracting audio & translating narrative script...');
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/translate-video`, {
        method: 'POST',
        body: formData
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || 'Failed to translate video.');
      }

      const scriptData = await response.json();
      setGeneratedScript(scriptData);
      onSelectScript(scriptData);
      setSuccessMsg('Video successfully translated & script storyboard created!');
      setFile(null);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-5 space-y-6">
        <div className="glass-panel rounded-2xl p-6 border-pink-500/20 shadow-neon-pink relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-cyber-pink/5 rounded-full blur-2xl pointer-events-none" />
          
          <h2 className="text-xl font-black text-white tracking-wide uppercase flex items-center gap-2 mb-1.5">
            <RefreshCw className="w-5 h-5 text-cyber-pink animate-spin" style={{ animationDuration: '4s' }} />
            Video Recreator
          </h2>
          <p className="text-xs text-cyber-muted mb-6 leading-relaxed">
            Upload any kind of video or audio file to transcribe its content, translate it into another language, and automatically generate a complete voiceover storyboard.
          </p>

          <form onSubmit={handleRecreateSubmit} className="space-y-4">
            {/* File Drag & Drop block */}
            <div className="border border-dashed border-slate-800 hover:border-cyber-pink/50 rounded-2xl p-6 bg-slate-950/40 text-center cursor-pointer transition-all relative group">
              <input 
                type="file" 
                onChange={handleFileChange}
                accept=".mp4,.mkv,.avi,.mov,.mp3,.wav,.m4a,.webm"
                required
                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
              />
              <div className="space-y-2">
                <Upload className="w-8 h-8 text-cyber-muted mx-auto group-hover:text-cyber-pink transition-colors" />
                <div className="text-xs font-semibold text-slate-300">
                  {file ? file.name : 'Select or drag any video file here'}
                </div>
                <div className="text-[10px] text-cyber-muted">
                  Supports MP4, MKV, AVI, MOV, MP3, WAV (Max 500MB)
                </div>
              </div>
            </div>
            ...
            {file && file.size > 50 * 1024 * 1024 && (
              <div className="bg-amber-950/40 border border-amber-500/20 rounded-xl p-3.5 text-[11px] text-amber-200 leading-relaxed animate-fade-in text-left">
                ⚠️ <strong>Large File Warning ({ (file.size / (1024 * 1024)).toFixed(0) }MB):</strong> Large uploads can exceed server request timeout rules on hosted platforms like Render (30-second cap). We downsample file snippets for speed, but uploading a compressed audio track (.mp3) is highly recommended for massive files!
              </div>
            )}

            {/* Settings */}
            <div className="border border-slate-900 rounded-xl overflow-hidden text-left">
              <button
                type="button"
                  ...
                  >
                    <div className="p-4 grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-cyber-pink uppercase tracking-wider block">Target Language</label>
                        <select
                          value={language}
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-cyber-pink uppercase tracking-wider block">Recreation Tone</label>
                        <select
                          value={tone}
                        ...
                        >
                          ...
                          <option value="Informative & Professional">Informative</option>
                          <option value="Conversational & Casual">Casual</option>
                          <option value="Action-Packed & Hype">Energetic</option>
                        </select>
                      </div>
                    </div>
                  </motion.div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-pink-500 to-purple-600 hover:opacity-90 disabled:opacity-50 text-xs font-black text-white uppercase tracking-wider py-3.5 rounded-xl shadow-lg transition-all"
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : <Sparkles className="w-4 h-4 text-white" />}
              <span>{isLoading ? 'Translating & recreating video...' : 'Translate & Recreate'}</span>
            </button>
          </form>
        </div>
        ...
        {/* Sidebar logs */}
        <ScriptHistory 
          scripts={scripts.filter(s => s.videoType === 'translated')} 
      ...
      <div className="lg:col-span-7 space-y-6">
        {error && (
          <div className="bg-red-950/60 border border-red-500/30 rounded-xl p-3.5 text-red-200 text-xs flex items-start gap-2 text-left">
        ...
        {successMsg && (
          <div className="bg-green-950/60 border border-green-500/40 rounded-xl p-3 text-green-200 text-xs flex items-center gap-2 text-left">
        ...
        {generatedScript ? (
          <motion.div
            ...
            className="space-y-4"
          >
            <div className="glass-panel rounded-2xl p-5 border-purple-500/20 shadow-inner text-left">
              ...
              <div className="flex gap-4 text-[10px] text-cyber-muted font-mono uppercase">
                ...
                <span>Type: Recreated</span>
              </div>
            </div>

            {/* Translation Actions */}
            <div className="bg-[#100b26]/60 border border-purple-500/10 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 animate-fade-in text-left">
            ...
            {generatedScript.metadata && (
              <div className="glass-panel rounded-2xl p-5 border-pink-500/20 bg-[#0d091e]/80 space-y-4 shadow-lg text-left">
            ...
            <div className="glass-panel rounded-2xl p-5 border-purple-500/20 bg-[#0c0817]/70 space-y-4 shadow-md text-left">
              <div className="flex items-center gap-2 pb-2 border-b border-slate-900">
                <Volume2 className="w-4.5 h-4.5 text-cyber-pink" />
            ...
            {(generatedScript.audioBase64 === 'active' || (generatedScript.audioBase64 && generatedScript.audioBase64 !== 'separate')) && (
              <div className="bg-[#0b0816] border border-cyan-500/10 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-left">
                <div className="flex items-center gap-3">
                  <div className="bg-cyan-500/10 border border-cyan-500/30 p-2 rounded-lg text-cyber-cyan">
                    <Volume2 className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider">Translated Voiceover Track</h4>
                    <p className="text-[10px] text-cyber-muted font-mono">Unified translated narration audio track</p>
                  </div>
                </div>
                <audio 
            ...
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1 text-left">
                  ...
                  <div className="space-y-2.5">
                    <div>
                      <span className="text-[9px] font-black text-cyber-cyan uppercase tracking-widest block mb-0.5">
                        Narration text ({generatedScript.language})
                      </span>
          ...
          <div className="glass-panel rounded-2xl p-8 flex flex-col items-center justify-center text-center border-pink-500/10 min-h-[450px]">
            <Video className="w-12 h-12 text-cyber-muted/50 mb-3 animate-pulse" />
            <h3 className="text-lg font-semibold text-slate-300">Translated Script Storyboard Output</h3>
            <p className="text-sm text-cyber-muted mt-1 max-w-sm">
              Upload generic videos, clips, or B-rolls to see translated script narration timelines and dialogue blocks appear here.
            </p>
          </div>
        )}
      </div>
